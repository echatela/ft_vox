...
const LabelMesh Printer::_createLabelMesh(const std::string& text, float size,
                                          glm::vec2         pos,
                                          const glm::ivec2& resolution) const
{
	...
	for (const char letter : text)
	{
		...
		label.coords.push_back(
		    glm::vec2((info.offset + info.width) / kFontWidth, 1));

		// v2
		label.coords.push_back(glm::vec2(endX, endY));
		label.coords.push_back(
		    glm::vec2((info.offset + info.width) / kFontWidth, 0));
...
unsigned int Printer::_generateGPUBuffers(const LabelMesh& label) const
{
	...
	glBufferData(GL_ARRAY_BUFFER, label.coords.size() * sizeof(glm::vec2),
	             label.coords.data(), GL_STATIC_DRAW);

	glGenBuffers(1, &EBO);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER,
	             label.indexes.size() * sizeof(unsigned int),
	             label.indexes.data(), GL_STATIC_DRAW);

	// layout 0 -> pos
	glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(glm::vec2) * 2,
	                      (void*)0);
	glEnableVertexAttribArray(0);

	// layout 1 -> UV
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(glm::vec2) * 2,
...
void Printer::_drawLabel(const LabelMesh& label, const glm::vec3& color) const
{
	...
	glDrawElements(GL_TRIANGLES, label.indexes.size(), GL_UNSIGNED_INT,
	               (void*)0);
}
